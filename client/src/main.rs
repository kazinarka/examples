...
fn main() {
    let matches = app_from_crate!()
        ...
        .subcommand(
            SubCommand::with_name("generate_random_number_v2")
                .arg(
                    Arg::with_name("sign")
                        .short("s")
                        .long("sign")
    ...
    if let Some(matches) = matches.subcommand_matches("generate_random_number_v2") {
        let program_id = PROGRAM_ID.parse::<Pubkey>().unwrap();
        ...
        let vrf = "AR6FpWt7HdutDRMqCWYk8UZfeby3w3c2s6nWBTcFw6MV".parse::<Pubkey>().unwrap();

        let instructions = vec![Instruction::new_with_borsh(
            program_id,
            &ExampleInstruction::GenerateRandomNumberV2 {max_result: 1000},
            vec![
