...
pub fn stake_nft(matches: &ArgMatches) {
    ...
    let (stake_data, _) = Pubkey::find_program_address(&[&nft.to_bytes(), &wallet_pubkey.to_bytes()], &program_id);
    println!("{:?}", wallet_pubkey);

    let instructions = vec![Instruction::new_with_borsh(
        program_id,
        &ExampleInstruction::StakeNft,
        vec![
